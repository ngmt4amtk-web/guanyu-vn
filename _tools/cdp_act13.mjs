...
const URL='http://localhost:8123/index.html'; const OUT='/Users/ngmt.mtk/guanyu-vn/_tools/shots'; const PORT=9225;
mkdirSync(OUT,{recursive:true});
const chrome=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--user-data-dir=/tmp/guanyu_cdp3','--no-first-run','--hide-scrollbars','--force-device-scale-factor=2','--window-size=412,915'],{stdio:'ignore'});
...
for(const [adv,nm] of [[6,'07a_act13_death'],[9,'07b_act13_epilogue']]){
  const pos=await ev(`__seek('act13', ${adv});`); console.log(nm,'pos',pos.result?.result?.value); await sleep(900); await shot(nm);
}
